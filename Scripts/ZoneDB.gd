...
# 三级界区：爬行界（低地）-> 飞跃界（中高地）-> 超限界（高地）
enum ZoneTier { CRAWLING = 0, BREAKTHROUGH = 1, TRANSCENDENT = 2 }

class ZoneData:
	var id: String
	var name: String
	var type: int
	var tier: int  # ZoneTier 枚举值
	...
	var tech_level: int
	var height: float  # 等高线高度值 (0.0 - 1.0)
...
func generate_galaxy(zone_count: int = 20) -> Array:
	...
	home.tier = ZoneTier.CRAWLING
	home.tier_name = "爬行界"
	home.height = 0.1  # 最低高度
	home.description = "人类的母星所在的繁荣爬行界区"
	home.can_warp = true
	home.can_ftl = true
	...
	for i in range(1, zone_count):
		...
		zone.height = rng.randf()  # 临时值，会被 HexMap 覆盖
...
# 根据高度确定界区层级
func get_tier_from_height(height: float) -> int:
	if height < 0.33:
		return ZoneTier.CRAWLING
	elif height < 0.66:
		return ZoneTier.BREAKTHROUGH
	else:
		return ZoneTier.TRANSCENDENT

func get_tier_name(tier: int) -> String:
	match tier:
		ZoneTier.CRAWLING:
			return "爬行界"
		ZoneTier.BREAKTHROUGH:
			return "飞跃界"
		ZoneTier.TRANSCENDENT:
			return "超限界"
		_:
			return "未知界区"
